...
const isXOrTwitterUrl = (url = "") => {
  const text = String(url || "").toLowerCase();

  return (
    text.includes("x.com/") ||
    text.includes("twitter.com/") ||
    text.includes("mobile.twitter.com/") ||
    text.includes("fxtwitter.com/") ||
    text.includes("vxtwitter.com/")
  );
};
...
const normalizeXUrl = (url = "") => {
  return String(url || "")
    ...
    .replace("https://vxtwitter.com/", "https://twitter.com/");
};

const getRefererForUrl = (url = "") => {
  if (isXOrTwitterUrl(url)) return "https://twitter.com/";
  if (isYouTubeUrl(url)) return "https://www.youtube.com/";
  return "";
};

const buildYtDlpBaseArgs = (originalUrl = "") => {
  const args = [
    "--no-playlist",
    "--newline",
    "--force-overwrites",
  ...
  if (isXOrTwitterUrl(originalUrl)) {
...
const normalizeErrorMessage = (stderr = "") => {
  ...
  if (isXError && isCookieError) {
    return {
      statusCode: 401,
      code: "X_LOGIN_OR_COOKIES_REQUIRED",
      error:
        "This X/Twitter video may require login or cookies. Try a public post URL or configure COOKIES_FILE on the server.",
    };
  }
  ...
  if (isXError) {
    return {
      statusCode: 422,
      code: "X_EXTRACT_FAILED",
      error:
        "X/Twitter could not fully process this link. Try another public post/video URL.",
    };
  }
  ...
  try {
    ...
    ytDlp.on("close", async (code) => {
      ...
      res.status(200).json({
        ...
        // Direct X/Twitter preview URLs may not play in every browser because of hotlink/CORS/expiry.
        // Use these for UI only; final download should go through downloadDirectMedia with originalUrl.
        previewUrl: bestPreview?.url || "",
        previewAudioUrl:
          bestPreview && !bestPreview.hasAudio ? bestPreview.audioUrl : "",
        previewHasAudio: Boolean(bestPreview?.hasAudio),
...
exports.downloadDirectMedia = async (req, res) => {
  ...
  try {
    ...
    const platformName = String(platform || "").toLowerCase();
    let originalUrlValue = String(originalUrl || "");

    if (isXOrTwitterUrl(originalUrlValue)) {
      originalUrlValue = normalizeXUrl(originalUrlValue);
    }

    const shouldUseYtDlp =
      originalUrlValue &&
      (platformName.includes("youtube") ||
        platformName.includes("twitter") ||
        platformName.includes("x") ||
        isYouTubeUrl(originalUrlValue) ||
        isXOrTwitterUrl(originalUrlValue));
    ...
    if (shouldUseYtDlp) {
      ...
      const args = [
        ...buildYtDlpBaseArgs(originalUrlValue),
        "-N",
        "4",
      ];
      ...
      if (type === "audio") {
        ...
        if (videoFormatId && selectedHasAudio) {
          ...
          formatSpec = `${videoFormatId}+${audioFormatId}/${videoFormatId}+bestaudio/best[ext=mp4]/best`;
        } else if (videoFormatId) {
          formatSpec = `${videoFormatId}+bestaudio/bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best`;
        }
      ...
      childProcess.on("close", (code, signal) => {
        ...
        if (code !== 0) {
          ...
          return sendJsonIfConnected(res, friendlyError.statusCode || 500, {
            status: "fail",
            code: friendlyError.code || "YTDLP_DOWNLOAD_FAILED",
            error:
              friendlyError.error ||
              "Download was not completed. Please try again.",
            details: cleanError,
          });
        }
        ...
        const files = fs
          ...
          .sort((a, b) => {
            ...
            return fs.statSync(bPath).size - fs.statSync(aPath).size;
          });
